...
import detectEthereumProvider from './teleport-decter';

type MetaMaskProvider = Provider & { isTeleportWallet?: boolean; isMetaMask?: boolean; isConnected?: () => boolean; providers?: MetaMaskProvider[] }

export class NoMetaMaskError extends Error {
  public constructor() {
    super('Teleport not installed')
...
export class Teleport extends Connector {
  ...
  private async isomorphicInitialize(): Promise<void> {
    ...
    return (this.eagerConnection = detectEthereumProvider().then(async (m) => {
      const provider = m
      if (provider) {
        ...
        if (this.provider.providers?.length) {
          this.provider = this.provider.providers.find((p) => p.isTeleportWallet) ?? this.provider.providers[0]
        }
